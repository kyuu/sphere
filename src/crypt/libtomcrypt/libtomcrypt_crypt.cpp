...
namespace crypt {
    ...
    {
        return Cipher::Create(type);
    }

    //-----------------------------------------------------------------
    IHash* CreateHash(int type)
    {
        return Hash::Create(type);
    }
    ...
    bool Base64Encode(const u8* buf, int len, Blob* out)
    {
        ...
        int out_size = (4 * ((len + 2) / 3))) + 1;
        if (!out->resize(out_size)) {
            return false;
        }
        int err = base64_encode(buf, len, out->getBuffer(), &out_size);
        if (err == CRYPT_BUFFER_OVERFLOW) {
            if (!out->resize(out_size)) {
                return false;
            }
            err = base64_encode(buf, len, out->getBuffer(), &out_size);
        }
        if (err == CRYPT_OK) {
            out->resize(out_size);
            return true;
    ...
    //-----------------------------------------------------------------
    u32 ComputeCRC32(const u8* buf, int len, u32 in_crc32)
    {
        if (!buf || len <= 0) {
            return 0;
        }
        u32 crc32 = in_crc32 ^ 0xFFFFFFFF;
        for (int i = 0; i < len; i++) {
