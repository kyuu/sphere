...
namespace sphere {
    ...
    File::open(const std::string& filename, int mode)
    {
        ...
        switch (mode) {
        case IFile::FM_IN:     _file = fopen(filename.c_str(), "rb"); break;
        case IFile::FM_OUT:    _file = fopen(filename.c_str(), "wb"); break;
        case IFile::FM_APPEND: _file = fopen(filename.c_str(), "ab"); break;
    ...
    {
        if (_file) {
            return (_mode == IFile::FM_OUT || _mode == IFile::FM_APPEND);
        }
        return false;
    }
