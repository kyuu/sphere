...
namespace input {
    ...
    bool IsKeyPressed(int key)
    {
        if (key >= 0 && key < NUM_KEYS) {
            return g_keyboard_state[key_to_sdl_scancode[key]] != 0;
        }
        return false;
    }
    ...
    int GetNumStorableForceEffects(int joy)
    {
        if (joy >= 0 && joy < (int)g_joysticks.size() && g_joysticks[joy].haptic) {
            return SDL_HapticNumEffects(g_joysticks[joy].haptic);
        }
        return 0;
    }
    ...
    {
        if (joy >= 0 && joy < (int)g_joysticks.size() && g_joysticks[joy].haptic) {
            return SDL_HapticNumEffectsPlaying(g_joysticks[joy].haptic);
        }
        return 0;
    }
    ...
    static bool setup_sdl_haptic_direction(const SDL_HapticDirection& sdl_haptic_direction, const ForceDirection& force_direction)
    {
        ...
        case ForceEffect::Direction::CARTESIAN:
            ...
            sdl_haptic_direction.dir[0] = (Sint32)force_direction.params[0];
            sdl_haptic_direction.dir[1] = (Sint32)force_direction.params[1];
            sdl_haptic_direction.dir[2] = (Sint32)force_direction.params[2];
            break;
    ...
    //-----------------------------------------------------------------
    bool PlayForceEffect(int joy, int effect, int iterations)
    {
        if (joy >= 0 && joy < (int)g_joysticks.size() && g_joysticks[joy].haptic) {
            Uint32 iters = (iterations == ForceEffect::INFINITY ? SDL_HAPTIC_INFINITY : iterations)
    ...
    namespace internal {
        ...
        bool InitInput(const Log& log)
        {
            ...
            if (num_joysticks > 0) {
                log.info() << num_joysticks << " Joystick(s) available";
                ...
                for (int i = 0; i < num_joysticks; ++i) {
                    ...
                    if (haptic) {
                        log.info() << "Joystick " << i << " supports Force Feedback";
                    } else {
                        log.info() << "Joystick " << i << " does not support Force Feedback";
                    }
