...
#define GET_ARG_VEC2(idx, name)                 Vec2f*          name = GetVec2(idx + 1);        if (!name) { return ThrowError("Invalid argument %d '%s', expected a Vec2 instance",        idx, #name); }
...
#define GET_OPTARG_VEC2(idx, name)              Vec2f*          name = 0;       if (sq_gettop(v) >= idx + 1) { name = GetVec2(idx + 1);         if (!name) { return ThrowError("Invalid argument %d '%s', expected a Canvas instance",      idx, #name); } }
...
#define RET_RECT(expr)          BindRect(expr);                 return 1;
#define RET_VEC2(expr)          BindVec2(expr);                 return 1;
#define RET_STREAM(expr)        BindStream(expr);               return 1;
#define RET_BLOB(expr)          BindBlob(expr);                 return 1;
#define RET_FILE(expr)          BindFile(expr);                 return 1;
#define RET_CANVAS(expr)        BindCanvas(expr);               return 1;
#define RET_TEXTURE(expr)       BindTexture(expr);              return 1;
#define RET_SOUND(expr)         BindSound(expr);                return 1;
#define RET_SOUNDEFFECT(expr)   BindSoundEffect(expr);          return 1;
#define RET_ZSTREAM(expr)       BindZStream(expr);              return 1;
